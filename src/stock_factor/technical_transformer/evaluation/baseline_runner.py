...
def _baseline_loss(
    ...
    *,
    ...
    if phase_rows.any():
        losses.append((
            -target_phase[phase_rows]
            * F.log_softmax(prediction_phase[phase_rows], dim=-1)
        ).sum(dim=-1).mean())
...
def run_baseline_runner(
    ...
    *,
    ...
    if len(train_dataset):
        event_values = np.stack([train_dataset[index][1][LABEL_SCHEMA.slices["events"]] for index in range(len(train_dataset))])
        event_valid = np.stack([train_dataset[index][2][LABEL_SCHEMA.slices["events"]] for index in range(len(train_dataset))])
    ...
    for name in names:
        ...
        for _epoch in range(epochs):
            ...
            for x, y, label_valid, _ in train_loader:
                ...
                loss = _baseline_loss(
                    model(x.to(target_device)), y.to(target_device),
                    label_valid.to(target_device, dtype=torch.bool), event_weight=event_weight,
                )
                loss.backward()
                optimizer.step()
    ...
    if "transformer" in result["scores"] and "gru" in result["scores"]:
        result["transformer_double_oos_composite_relative_gain"] = relative_gain(result["scores"]["transformer"], result["scores"]["gru"])
    return result
