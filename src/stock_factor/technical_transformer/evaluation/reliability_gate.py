...
DEFAULT_GATES: dict[str, Any] = {
    ...
    "boll": {
        "percent_b_pearson_min": 0.95,
        "bandwidth_pearson_min": 0.92,
        "squeeze_spearman_min": 0.90,
        "expansion_spearman_min": 0.82,
    },
    "phase": {"macro_f1_min": 0.70},
    "wyckoff": {
        "primitive_spearman_min": 0.75,
        "gold_event_relative_pr_min": 2.0,
        "double_oos_event_degradation_max": 0.50,
    },
    "gold": {
        "kappa_min": 0.60,
        "allowed_splits": ["double_oos"],
        "require_oos_only": True,
        "require_coverage": True,
        "min_positive_per_event": 100,
        "min_negative_per_event": 200,
    },
    "oos": {"time_degradation_max": 0.15, "instrument_degradation_max": 0.20, "double_oos_degradation_max": 0.25},
    "embedding": {
        "noise_cosine_min": 0.95,
        "price_scale_cosine_min": 0.98,
        "nearest_neighbor_semantic_hit_min": 0.70,
        "weak_phase_neighbor_hit_min": 0.70,
        "gold_neighbor_semantic_hit_min": 0.70,
    },
    "invariance": {
        "require_raw_source": True,
        "raw_noise_embedding_cosine_min": 0.95,
        "raw_noise_phase_js_max": 0.10,
        "raw_noise_event_median_delta_max": 0.10,
    },
    "baseline": {
        "transformer_wyckoff_gold_relative_gain_min": 0.05,
        "transformer_double_oos_composite_relative_gain_min": 0.05,
    },
...
def load_gate_config(path: str | Path | None) -> dict[str, Any]:
    ...
    value = load_config(path).payload
...
def _merge(base: dict, override: dict | None) -> dict:
    ...
    for old, new in (
        ("slope_mean_sign_accuracy_min", "slope_sign_accuracy_min"),
        ("slope_mean_pearson_min", "slope_pearson_min"),
    ):
    ...
    if "gold_event_pr_auc_multiple_of_prevalence_min" in result.get("wyckoff", {}):
        result["wyckoff"]["gold_event_relative_pr_min"] = result["wyckoff"][
            "gold_event_pr_auc_multiple_of_prevalence_min"
        ]
    return result
...
def evaluate_reliability_gate(report: dict[str, Any], gates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    shortcut_actual = (
        len(leakage.get("violations", [])) if isinstance(leakage, dict) and "violations" in leakage else None
    )
    ...
    check(
        "data.future_leakage_violations",
        _value(causality, ("total_violations",)),
        thresholds["data"]["future_leakage_violations_max"],
        "<=",
    )
    split_overlap = _value(
        report,
        ("data_integrity", "split", "overlap"),
        _value(report, ("data", "split_overlap"), _value(report, ("dataset", "split_overlap"))),
    )
    ...
    check(
        "ma.slope_sign_accuracy",
        summary.get("ma_slope_mean_sign_accuracy"),
        thresholds["ma"]["slope_sign_accuracy_min"],
    )
    ...
    if alignment is None:
        alignment_values = [
            test.get("ma", {}).get(name, {}).get("spearman")
            for name in ("bull_alignment_score", "bear_alignment_score")
        ]
    ...
    check(
        "boll.percent_b_pearson",
        summary.get("bollinger_percent_b_pearson", boll.get("percent_b", {}).get("pearson")),
        thresholds["boll"]["percent_b_pearson_min"],
    )
    check(
        "boll.bandwidth_pearson",
        summary.get("bollinger_bandwidth_pearson", boll.get("bandwidth", {}).get("pearson")),
        thresholds["boll"]["bandwidth_pearson_min"],
    )
    check(
        "boll.squeeze_spearman",
        summary.get("bollinger_squeeze_spearman", boll.get("squeeze_score", {}).get("spearman")),
        thresholds["boll"]["squeeze_spearman_min"],
    )
    check(
        "boll.expansion_spearman",
        summary.get("bollinger_expansion_spearman", boll.get("boll_expansion_score", {}).get("spearman")),
        thresholds["boll"]["expansion_spearman_min"],
    )
    check(
        "wyckoff.primitive_spearman",
        summary.get("wyckoff_primitive_mean_spearman"),
        thresholds["wyckoff"]["primitive_spearman_min"],
    )
    phase_metrics = test.get("phase", {}).get("aggregate", {})
    check(
        "phase.macro_f1",
        summary.get("phase_macro_f1", phase_metrics.get("macro_f1")),
        thresholds["phase"]["macro_f1_min"],
    )
    ...
    if "kappa" in gold:
        ...
        checks["gold.kappa"] = {
            "passed": False,
            "actual": None,
            "threshold": thresholds["gold"]["kappa_min"],
            "operator": ">=",
        }
    if "allowed_split_passed" in gold:
        checks["gold.allowed_split"] = {
            "passed": bool(gold.get("allowed_split_passed")),
            "actual": gold.get("allowed_splits"),
            "threshold": thresholds["gold"].get("allowed_splits", ["double_oos"]),
            "operator": "allowed",
        }
    ...
    if "coverage_passed" in gold:
        checks["gold.coverage"] = {
            "passed": bool(gold.get("coverage_passed")),
            "actual": gold.get("coverage"),
            "threshold": "all events covered",
            "operator": "complete",
        }
    ...
    check(
        "wyckoff.gold_event_relative_pr",
        _value(gold, ("event", "pr_auc_multiple_of_prevalence")),
        thresholds["wyckoff"]["gold_event_relative_pr_min"],
    )

    oos = report.get("oos", {})
    check("oos.time_degradation", _value(oos, ("time_degradation",)), thresholds["oos"]["time_degradation_max"], "<=")
    check(
        "oos.instrument_degradation",
        _value(oos, ("instrument_degradation",)),
        thresholds["oos"]["instrument_degradation_max"],
        "<=",
    )
    check(
        "oos.double_oos_degradation",
        _value(oos, ("double_oos_degradation",)),
        thresholds["oos"]["double_oos_degradation_max"],
        "<=",
    )
    event_degradation = _value(
        oos, ("groups", "event", "double_oos_degradation"), _value(oos, ("groups", "events", "double_oos_degradation"))
    )
    check(
        "oos.double_oos_event_degradation",
        event_degradation,
        thresholds["wyckoff"]["double_oos_event_degradation_max"],
        "<=",
    )

    embedding = report.get("embedding", {})
    weak_neighbor = _value(
        embedding, ("weak_phase_neighbor_hit",), _value(embedding, ("nearest_neighbor_semantic_hit",))
    )
    check(
        "embedding.weak_phase_neighbor_hit",
        weak_neighbor,
        thresholds["embedding"].get(
            "weak_phase_neighbor_hit_min", thresholds["embedding"]["nearest_neighbor_semantic_hit_min"]
        ),
    )
    gold_neighbor = _value(embedding, ("gold_neighbor_semantic_hit",))
    if gold_neighbor is not None:
        check(
            "embedding.gold_neighbor_semantic_hit",
            gold_neighbor,
            thresholds["embedding"]["gold_neighbor_semantic_hit_min"],
        )
    elif "gold_neighbor_semantic_hit" in embedding or mode == "PRODUCTION":
        checks["embedding.gold_neighbor_semantic_hit"] = {
            "passed": False,
            "actual": None,
            "threshold": thresholds["embedding"]["gold_neighbor_semantic_hit_min"],
            "operator": ">=",
        }
    invariance = report.get("invariance", {}) or embedding.get("invariance", {})
    raw_source = invariance.get("raw_source_available")
    if raw_source is not None:
        checks["invariance.raw_source"] = {
            "passed": bool(raw_source),
            "actual": raw_source,
            "threshold": True,
            "operator": "required",
        }
    ...
    if raw_noise or require_raw:
        ...
        check(
            "invariance.raw_noise_embedding_cosine",
            _value(raw_noise, ("embedding_cosine",)),
            thresholds["invariance"]["raw_noise_embedding_cosine_min"],
        )
        check(
            "invariance.raw_noise_phase_js",
            _value(raw_noise, ("phase_js_divergence",)),
            thresholds["invariance"]["raw_noise_phase_js_max"],
            "<=",
        )
        check(
            "invariance.raw_noise_event_median_delta",
            _value(raw_noise, ("event_median_probability_delta",)),
            thresholds["invariance"]["raw_noise_event_median_delta_max"],
            "<=",
        )
        check(
            "embedding.price_scale_cosine",
            _value(invariance, ("price_scale_cosine",)),
            thresholds["embedding"]["price_scale_cosine_min"],
        )
        feature_noise = invariance.get("feature_noise_invariance") or {}
        check(
            "embedding.feature_noise_cosine",
            _value(feature_noise, ("cosine",)),
            thresholds["embedding"]["noise_cosine_min"],
        )
    else:
        # Compatibility for first-round reports; v3 reports use raw_noise_invariance above.
        check(
            "embedding.noise_cosine",
            _value(invariance, ("noise_cosine",), _value(invariance, ("cosine",))),
            thresholds["embedding"]["noise_cosine_min"],
        )
        check(
            "embedding.price_scale_cosine",
    ...
    transformer_gain = _value(
        baseline,
        ("transformer_double_oos_composite_relative_gain",),
        _value(baseline, ("transformer_wyckoff_gold_relative_gain",)),
    )
    baseline_threshold = thresholds["baseline"].get(
        "transformer_double_oos_composite_relative_gain_min",
        thresholds["baseline"]["transformer_wyckoff_gold_relative_gain_min"],
    )
    ...
    checks["baseline.transformer_gain"] = {
        "passed": True,
        "actual": transformer_gain,
        "threshold": baseline_threshold,
        "operator": ">=",
        "hard_gate": False,
        "warning": None if baseline_meets_threshold else "BASELINE_GAIN_BELOW_RESEARCH_THRESHOLD",
    }
    ...
    for name, required_flag in required.items():
        ...
        present = (
            ...
            and evidence.get("status") == "EVALUATED"
            and evidence.get("passed", True) is not False
        )
        if name == "double_oos":
            present = int(evidence.get("sample_count", 0)) > 0
        if name == "causality":
            present = present and "total_violations" in evidence
        checks[f"evidence.{name}"] = {
            "passed": bool(present),
            "actual": evidence.get("status") if isinstance(evidence, dict) else None,
            "threshold": "present",
            "operator": "present",
        }
    ...
    return {
        ...
        "gate_version": thresholds.get("version", "technical-reliability-gate.v1"),
...
_ALLOWED_TRANSITIONS = {
    ...
    "TESTED": {"RELIABILITY_PASSED", "REJECTED"},
    "RELIABILITY_PASSED": {"ACTIVE", "REJECTED"},
    "ACTIVE": set(),
    "REJECTED": set(),
}
