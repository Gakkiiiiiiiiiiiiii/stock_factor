...
class LastDayMLP(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_size: int = 128) -> None:
        super().__init__()
        self.network = nn.Sequential(nn.LayerNorm(input_dim), nn.Linear(input_dim, hidden_size), nn.GELU(), nn.Linear(hidden_size, output_dim))
...
class TCNBaseline(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_size: int = 128) -> None:
        ...
        self.network = nn.Sequential(
            nn.Conv1d(input_dim, hidden_size, kernel_size=3, padding=2, dilation=1), nn.GELU(),
            nn.Conv1d(hidden_size, hidden_size, kernel_size=5, padding=4, dilation=2), nn.GELU(),
        )
        self.head = nn.Linear(hidden_size, output_dim)
...
class TransformerBaseline(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_size: int = 128, layers: int = 2, heads: int = 4) -> None:
        super().__init__()
...
def make_baseline(name: str, input_dim: int, output_dim: int, **kwargs) -> nn.Module:
    values = {"mlp": LastDayMLP, "last_day_mlp": LastDayMLP, "gru": GRUBaseline, "tcn": TCNBaseline, "transformer": TransformerBaseline}
    try:
...
def compare_baselines(scores: dict[str, float], *, transformer_name: str = "transformer", baseline_name: str = "gru") -> dict[str, float | bool]:
    ...
    return {"transformer_score": float(scores[transformer_name]), "baseline_score": float(scores[baseline_name]), "relative_gain": gain, "passes_five_percent_gain": gain >= 0.05}
