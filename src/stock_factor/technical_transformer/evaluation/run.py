...
from .invariance import (
    add_small_ohlc_noise,
    model_embedding_invariance,
    model_event_probability_delta,
    model_phase_js_divergence,
    transform_price_scale,
)
...
def _collect_embeddings(
    model: torch.nn.Module, dataset: TechnicalWindowDataset, device: torch.device, *, limit: int = 2000
) -> dict[str, np.ndarray]:
    ...
    if not embeddings:
        return {
            ...
            "targets": np.empty((0, len(LABEL_SCHEMA.names))),
            "valid": np.empty((0, len(LABEL_SCHEMA.names))),
            "raw": np.empty((0, len(FEATURE_NAMES))),
        }
    return {
...
def _embedding_evidence(
    model: torch.nn.Module, dataset_dir: Path, device: torch.device, *, gold: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    task_columns = {
        "ma_alignment": "bull_alignment_score",
        "trend_direction": "trend_direction",
        "boll_squeeze": "squeeze_score",
        "trading_range": "trading_range_score",
        "weak_spring": "spring_score",
        "weak_upthrust": "upthrust_score",
    }
    ...
    train_valid = {
        task: train["valid"][:, LABEL_SCHEMA.names.index(name)].astype(bool) for task, name in task_columns.items()
    }
    ...
    for split in ("time_test", "instrument_test", "double_oos"):
        ...
        test_valid = {
            task: test["valid"][:, LABEL_SCHEMA.names.index(name)].astype(bool) for task, name in task_columns.items()
        }
        ...
        probe = run_embedding_probe(
            ...
            train_raw_features=train["raw"],
            test_raw_features=test["raw"],
        )
        split_results[split] = {
            "status": "EVALUATED",
            **probe,
            "weak_phase_neighbor_hit": weak_phase_neighbor_hit(
                test["embedding"],
                test_targets["phase"],
                k=min(20, max(1, len(test["embedding"]) - 1)),
            ),
        }
    ...
    *,
    ...
    if not source_path or not Path(source_path).exists():
        return {
            ...
            "passed": False,
            "reason": "RAW_SNAPSHOT_NOT_AVAILABLE",
        }
    source = pd.read_parquet(source_path)
    processor_data = manifest.get("processor") or json.loads(
        (dataset_dir / "processor.json").read_text(encoding="utf-8")
    )
    ...
    groups = {
        str(symbol): group.sort_values("trading_date").reset_index(drop=True)
...
def _ablation_evidence(
    model: torch.nn.Module, dataset_dir: Path, device: torch.device, *, limit: int = 64
) -> dict[str, Any]:
    dataset = TechnicalWindowDataset(dataset_dir, "double_oos")
    if not len(dataset):
        return {"status": "NOT_EVALUATED", "reason": "EMPTY_DOUBLE_OOS"}
    windows = torch.from_numpy(
        np.asarray([dataset[index][0] for index in range(min(len(dataset), limit))], dtype=np.float32)
    ).to(device)
    model.eval()
    with torch.no_grad():
    ...
    return {
        "status": "EVALUATED",
        "full_event_score": full,
        "no_volume_event_score": no_volume,
        "event_score_delta": delta,
        "warnings": ["WYCKOFF_VOLUME_NOT_USED"] if delta < 0.01 else [],
    }


def _causality_evidence(
    model: torch.nn.Module, dataset_manifest: dict[str, Any], device: torch.device, cases: int
) -> dict[str, Any]:
    source_path = dataset_manifest.get("source_market_snapshot_path")
    if not source_path or not Path(source_path).exists():
        return run_causality_suite(
            None, feature_builder=build_features, label_builder=build_labels, model=model, cases=cases
        )
    frame = pd.read_parquet(source_path)
    processor_data = dataset_manifest.get("processor") or json.loads(
        (Path(dataset_manifest["series_path"]).parent / "processor.json").read_text(encoding="utf-8")
    )
    processor = RobustFeatureProcessor()
    processor.median = np.asarray(processor_data["median"], dtype=float)
    processor.scale = np.asarray(processor_data["scale"], dtype=float)
...
def run_reliability_evaluation(
    *,
    ...
    checkpoint_file = next(
        (checkpoint_dir / name for name in ("model.safetensors", "model.pt") if (checkpoint_dir / name).is_file()),
        None,
    )
    if checkpoint_file is None:
        raise FileNotFoundError("checkpoint directory has no model.safetensors or model.pt")
    split_manifest = manifest.get("split_manifest") or {
        key: manifest.get(key) for key in ("splits", "split_overlap", "split_config") if key in manifest
    }
    identity = {
        **identity,
        ...
        "label_schema_hash": manifest.get("label_schema_hash"),
        "training_snapshot_id": identity.get("market_snapshot_id") or manifest.get("source_market_snapshot_id"),
        "split_manifest_hash": manifest.get("split_manifest_hash")
        or hashlib.sha256(json.dumps(split_manifest, sort_keys=True, separators=(",", ":")).encode()).hexdigest(),
    }
    ...
    splits = evaluate_all_splits(
        checkpoint_dir, dataset_dir, device=str(target_device), batch_size=batch_size, max_batches=max_batches
    )
    causality = (
        _causality_evidence(model, manifest, target_device, causality_cases)
        if str(mode).upper() != "SMOKE"
        else {"status": "NOT_EVALUATED", "total_violations": None, "cases": 0}
    )
    gold = (
        evaluate_gold_set(
            model,
            ...
            min_positive_per_event=int(
                gates["gold"].get("min_positive_per_event", 100 if str(mode).upper() == "PRODUCTION" else 30)
            ),
            min_negative_per_event=int(
                gates["gold"].get("min_negative_per_event", 200 if str(mode).upper() == "PRODUCTION" else 50)
            ),
        )
        if gold_set and str(mode).upper() != "SMOKE"
        else {"status": "NOT_PROVIDED" if not gold_set else "NOT_EVALUATED"}
    )
    embedding = (
        _embedding_evidence(model, dataset_dir, target_device, gold=gold)
        if str(mode).upper() != "SMOKE"
        else {"status": "NOT_EVALUATED"}
    )
    invariance = (
        _invariance_evidence(model, dataset_dir, target_device, mode=mode)
        if str(mode).upper() != "SMOKE"
        else {"status": "NOT_EVALUATED"}
    )
    ablation = (
        _ablation_evidence(model, dataset_dir, target_device)
        if str(mode).upper() != "SMOKE"
        else {"status": "NOT_EVALUATED"}
    )
    ...
    if str(mode).upper() != "SMOKE":
        ...
        baseline = run_baseline_runner(
            dataset_dir,
            names=("gru", "last_day_mlp"),
            device=target_device,
            output_dir=baseline_root,
            baseline_config=baseline_cfg,
            epochs=int(baseline_cfg.get("max_epochs", 10)),
            patience=int(baseline_cfg.get("patience", 3)),
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--mode", default="PRODUCTION", choices=["SMOKE", "RESEARCH", "PRODUCTION", "smoke", "research", "production"]
    )
    ...
    result = run_reliability_evaluation(
        ...
        causality_cases=args.causality_cases,
    )
    print(
        json.dumps(
            {
                "json": str(Path(args.report) / "reliability_report.json")
                if Path(args.report).suffix == ""
