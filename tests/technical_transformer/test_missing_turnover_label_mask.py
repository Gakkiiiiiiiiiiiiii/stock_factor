...
def test_placeholder_turnover_never_becomes_effort_negative() -> None:
    rows = 240; close = np.linspace(10, 13, rows)
    frame = pd.DataFrame({"trading_date": pd.date_range("2020-01-01", periods=rows, freq="B"), "open": close - .02, "high": close + .05, "low": close - .05, "close": close, "volume": 100000., "amount": close * 100000., "turnover": .01, "turnover_observed": True})
