...
def _frame(rows: int = 150) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        "trading_date": pd.date_range("2020-01-01", periods=rows, freq="B"), "symbol": "AAA",
        "open": close, "high": close + 1, "low": close - 1, "close": close,
        "volume": 100.0, "amount": close * 100.0, "turnover": 0.01,
    })
...
def test_causality_detects_intentional_lookahead() -> None:
    ...
    result = run_causality_suite(
        _frame(), feature_builder=lookahead, label_builder=build_labels, cases=3, seed=4,
    )
