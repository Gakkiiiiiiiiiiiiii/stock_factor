...
def test_causality_runner_reports_real_prefix_cases() -> None:
    rows = 150; close = np.linspace(10, 20, rows)
    frame = pd.DataFrame({"trading_date": pd.date_range("2020-01-01", periods=rows, freq="B"), "symbol": "A", "open": close, "high": close + 1, "low": close - 1, "close": close, "volume": 100.0, "amount": close * 100, "turnover": 0.01})
