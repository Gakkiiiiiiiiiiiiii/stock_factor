...
from stock_factor.technical_transformer.evaluation.reliability_gate import evaluate_reliability_gate, transition_checkpoint_status  # noqa: E402
...
def _frame(rows: int = 240) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        "trading_date": pd.date_range("2022-01-01", periods=rows, freq="B"), "symbol": "AAA.SZ",
...
def test_mask_reconstruction_target_shape() -> None:
    ...
    model = TechnicalTransformerSystem({"input_dim": len(FEATURE_NAMES), "hidden_size": 32, "layers": 1, "heads": 4, "ffn_size": 64, "embedding_dim": 16})
    ...
    assert torch.isfinite(compute_mask_loss(output["mask_prediction"], masked.target, masked.positions, target_indices=indices))
